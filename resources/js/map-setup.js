...
function mapHandler(){
    ...
    const objectTypesInputs = mapFilter.querySelectorAll('[name=object_types]');
    const filterInputs = mapFilter.querySelectorAll('[data-name=filter-input]');

    const price_from = mapFilter.price_from;
    const price_to = mapFilter.price_to;

    const license_type = mapFilter.license_type;
    ...
    filterInputs.forEach(input => {
        input.addEventListener('change', () => {
            ...
            objectTypesInputs.forEach(input => {
